...
func (v *VictimBase) Pods(clientset kube.Interface) ([]corev1.Pod, error) {
	...
	podlist, err := clientset.CoreV1().Pods(v.namespace).List(getContext(context.TODO()), *labelSelector)
	if err != nil {
		return nil, err
	}
...
func (v *VictimBase) DeletePod(clientset kube.Interface, podName string) error {
	...
	return clientset.CoreV1().Pods(v.namespace).Delete(getContext(context.TODO()), podName, deleteOpts)
}
