...
func getContext(c context.Context) context.Context {
	ctx, _ := context.WithTimeout(c, time.Duration(*config.GracePeriodSeconds()*1000)) // want to use time.seconds
	return ctx
}
...
func EligibleStatefulSets(clientset kube.Interface, namespace string, filter *metav1.ListOptions) (eligVictims []victims.Victim, err error) {
	enabledVictims, err := clientset.AppsV1().StatefulSets(namespace).List(getContext(context.TODO()), *filter)
	if err != nil {
		return nil, err
	}
...
func (ss *StatefulSet) IsEnrolled(clientset kube.Interface) (bool, error) {
	statefulset, err := clientset.AppsV1().StatefulSets(ss.Namespace()).Get(getContext(context.TODO()), ss.Name(), metav1.GetOptions{})
	if err != nil {
		return false, err
	}
