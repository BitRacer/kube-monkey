...
func EligibleDaemonSets(clientset kube.Interface, namespace string, filter *metav1.ListOptions) (eligVictims []victims.Victim, err error) {
	enabledVictims, err := clientset.AppsV1().DaemonSets(namespace).List(getContext(context.TODO()), *filter)
	if err != nil {
		return nil, err
	}
...
func (d *DaemonSet) IsEnrolled(clientset kube.Interface) (bool, error) {
	daemonset, err := clientset.AppsV1().DaemonSets(d.Namespace()).Get(getContext(context.TODO()), d.Name(), metav1.GetOptions{})
	if err != nil {
		return false, err
	}
